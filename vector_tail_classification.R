#Note that classwise_pca need to be run with number_of_components_to_save changed to 784. 
pcs_to_include = c(5, 10, 20, 30, 60, 90, 120, 150, 180, 250, 350, 500)
...
for(included_pc in pcs_to_include){
  ...
  for(i in 1:60000){
    for(class_index in 1:10){
      tail_distance[i,class_index] <- sum(pca_coeffs_array[i,class_index,included_pc:(dim(pca_coeffs_array)[3])]^2)
    }
...
for(i in 1:length(test_labels)){
  for(class_index in 1:10){
    test_distance[i, class_index] <- sum(test_pca_coeffs_array[i,class_index,best_pc:(dim(test_pca_coeffs_array)[3])]^2)
  }
