...
for(validation_index in 1:K){
  ...
  for(class_index in 1:length(class_names)){
    ...
    for(sample_index in 1:test_images_per_validation){
      removed_distances[sample_index, class_index] <- norm_to_use(removed_data[sample_index, ] - cross_val_means[validation_index, class_index,])
    }
    for(sample_index in 1:training_images_per_validation){
      remaining_distances[sample_index, class_index] <- norm_to_use(remaining_data[sample_index,] - cross_val_means[validation_index, class_index,])
    }
...
print(mean(removed_accuracy))
print(mean(remaining_accuracy))
